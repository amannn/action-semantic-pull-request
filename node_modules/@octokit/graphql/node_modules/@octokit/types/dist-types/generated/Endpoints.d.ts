...
declare type Operation<Url extends keyof paths, Method extends keyof paths[Url], preview = unknown> = {
    ...
    response: Url extends keyof EndpointsWithMissingRequiredResponseDataSchema ? Method extends EndpointsWithMissingRequiredResponseDataSchema[Url] ? DeepRequired<ExtractFirstSuccessResponse<paths[Url][Method]>> : ExtractFirstSuccessResponse<paths[Url][Method]> : ExtractFirstSuccessResponse<paths[Url][Method]>;
};
...
declare type FirstSuccessResponse<R, S extends keyof R = keyof R> = S extends keyof SuccessStatusesMap ? R[S] extends never ? Omit<OctokitResponse<unknown, SuccessStatusesMap[S]>, "data"> : OctokitResponse<DataType<R[S]>, SuccessStatusesMap[S]> : S extends keyof RedirectStatusesMap ? OctokitResponse<unknown, RedirectStatusesMap[S]> : never;
